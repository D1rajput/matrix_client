...
export class CryptoUtil {
  ...
  encryptWithIvSalt(salt, iv, passPhrase, plainText) {
    ...
    return this.encrypted.ciphertext.toString(CryptoJS.enc.Base64);
  }
  ...
  encrypt(plainText) {
    if (null != plainText) {
      this.encryptPlainText(this.passPhrase, plainText);
    }
  }
  encryptPlainText(passPhrase, plainText) {
  ...
  decrypt(cipherText) {
    if (null != cipherText) {
      return this.decryptCipherText(this.passPhrase, cipherText);
    }
  }

  decryptCipherText(passPhrase, cipherText) {
